...
with gr.Blocks(title="천체사진 처리 도구 v0.12 (탭2 다크 로직 개선)", theme=gr.themes.Soft()) as app:
    logger_main.info("Gradio Blocks UI 정의 시작.")
    gr.Markdown("# 천체사진 처리 도구 v0.12 (탭2 다크 로직 개선)")
    gr.Markdown("탭을 선택하여 원하는 작업을 수행하세요. 로그는 콘솔 및 각 탭의 로그 창에 출력됩니다.")

    # 상태 변수
    state_master_bias_path = gr.State(None)
    state_master_darks_corrected_dict = gr.State({}) # Key: exp_time (float), Value: path
    state_master_flats_corrected_dict = gr.State({}) # Key: (filter_str, exp_time_float), Value: path
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("1. 마스터 프레임 생성"):
            ...
            # handle_tab1_master_frame_creation 반환값 순서 변경됨
        ...
        with gr.TabItem("2. LIGHT 프레임 보정"):
            ...
            # 탭 2 버튼 클릭 이벤트 수정: inputs 리스트에서 불필요한 None 제거 및 state_mf_dict_corr 전달
        ...
        with gr.TabItem("4. 상세 측광 및 카탈로그 분석"):
            ...
            tab4_process_button.click(
                ...
                outputs=[tab4_dataframe_output, tab4_csv_download_button, tab4_log_output]
            )
