    ...
    with gr.Tabs():
        ...
        with gr.TabItem("1. 마스터 프레임 생성 (ccdproc)"): 
            ...
            tab1_process_button.click(
                ...
                outputs=[ # 총 7개 반환값에 맞춰 UI 컴포넌트 + 상태 변수 매핑
                    tab1_bias_output_ui,              
                    tab1_darks_output_ui,             
                    tab1_flats_output_ui,             
            ...
            with gr.Row():
                with gr.Column(scale=3): 
                    ...
                    with gr.Accordion("(선택) 마스터 프레임 직접 업로드", open=False):
                        ...
                        tab2_uploaded_flat_b_obj = gr.File(label="Master FLAT B (Raw 또는 예비 플랫)", file_count="single", type="filepath") # 설명 변경
                        tab2_uploaded_flat_v_obj = gr.File(label="Master FLAT V (Raw 또는 예비 플랫)", file_count="single", type="filepath") # 설명 변경
                    with gr.Row():
            ...
            with gr.Row():
                with gr.Column(scale=2): 
                    ...
                    with gr.Accordion("마스터 프레임 업로드 (선택 사항, Raw 권장)", open=False):
                        ...
                        tab3_md_raw_files_input = gr.File(label="Master DARK (Raw, 다수, 노출시간별)", file_count="multiple", type="filepath") # 다중 파일로 변경
                        tab3_mf_b_raw_input = gr.File(label="Master FLAT B (Raw, 예비 플랫으로 사용)", file_count="single", type="filepath") # 설명 변경
                        tab3_mf_v_raw_input = gr.File(label="Master FLAT V (Raw, 예비 플랫으로 사용)", file_count="single", type="filepath") # 설명 변경
...
if __name__ == "__main__":
    ...
    os.system("ulimit -n 5000")  # 파일 핸들 제한 증가 (Linux/MacOS)
    import atexit
