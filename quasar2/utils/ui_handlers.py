...
from utils.calibration import (
    create_master_bias, create_master_dark, 
    create_master_flat, # 정석적인 마스터 플랫 생성 함수
    calibrate_light_numpy # 수동 보정 함수
)
...
# from astropy.nddata import CCDData # NumPy 배열을 직접 사용하므로 CCDData 객체 변환은 최소화
...
def handle_tab1_master_frame_creation(bias_file_objs, dark_file_objs, flat_file_objs_all, temp_dir):
    """
    탭 1: 마스터 프레임 생성 핸들러 (NumPy 기반).
    BIAS, DARK (노출시간별, BIAS 보정됨), 
    FLAT (필터별, BIAS/DARK 보정 및 정규화됨)을 생성하고 저장합니다.
    생성된 모든 마스터 프레임 파일 경로 리스트를 UI에 전달합니다.
    """
    status_messages = []
    ...
    state_master_darks_dict_out = {} # {exp_time: dark_path}
    state_master_flats_dict_out = {} # {filter_name: flat_path}
    
    master_bias_data_np = None # NumPy 배열로 BIAS 데이터 저장
    master_bias_header = None  # BIAS 헤더 저장용
    ...
    if bias_file_objs:
        try:
            ...
            if bias_file_paths:
                status_messages.append(f"BIAS: {len(bias_file_paths)}개 파일로 Master BIAS 생성 시작...")
                # create_master_bias는 NumPy 배열을 반환
                master_bias_data_np = create_master_bias(bias_file_paths) 
                if master_bias_data_np is not None:
                    # 헤더는 첫 번째 BIAS 파일의 것을 사용하거나 새로 생성
                    _, temp_bias_header = load_single_fits_from_path(bias_file_paths[0])
                    master_bias_header = temp_bias_header if temp_bias_header else fits.Header()
                    
                    saved_path = save_fits_image(master_bias_data_np, master_bias_header, "master_bias_numpy", temp_dir, current_timestamp_str)
                    if saved_path: 
                ...
                else: status_messages.append("BIAS: NumPy 기반 마스터 BIAS 생성 실패.")
            else: status_messages.append("BIAS: 유효한 파일 경로 없음.")
        except Exception as e: logger_ui.error("BIAS 처리 중 오류", exc_info=True); status_messages.append(f"BIAS 처리 오류: {str(e)}"); master_bias_data_np = None
    else: status_messages.append("BIAS: 업로드된 파일 없음.")

    # 2. Master DARK (노출 시간별 Corrected) 생성
    master_darks_data_for_flat_calib_dict = {} # 플랫 보정 시 사용할 {exp: (data, header)}
    if dark_file_objs:
        status_messages.append(f"DARK: 총 {len(dark_file_objs)}개 파일 처리 시작...")
        grouped_dark_paths_by_exp = {} 
        for df_obj in dark_file_objs: 
            if df_obj and df_obj.name and os.path.exists(df_obj.name):
                _, header = load_single_fits_from_path(df_obj.name)
                if header:
        ...
        for exp_time, dark_paths_list in grouped_dark_paths_by_exp.items():
            ...
            try:
                ...
                # create_master_dark는 (data_np, header) 반환
                master_dark_data_np, first_dark_header = create_master_dark(dark_paths_list, master_bias_data_np) 
                if master_dark_data_np is not None:
                    header_to_save = first_dark_header if first_dark_header else fits.Header()
                    if 'EXPTIME' not in header_to_save: header_to_save['EXPTIME'] = exp_time 
                    
                    base_fn = f"master_dark_exp{exp_time:.2f}s_numpy".replace('.', '_')
                    saved_path = save_fits_image(master_dark_data_np, header_to_save, base_fn, temp_dir, current_timestamp_str)
                    if saved_path:
                        state_master_darks_dict_out[exp_time] = saved_path 
                        ui_darks_paths_out.append(saved_path) 
                        master_darks_data_for_flat_calib_dict[exp_time] = (master_dark_data_np, header_to_save) # 플랫 보정용으로 데이터도 저장
                ...
                else: status_messages.append(f"Master DARK (Exp: {exp_time}s): 생성 실패.")
    ...
    # 3. 최종 Master FLAT (필터별) 생성 (BIAS/DARK 보정 및 정규화 완료)
    if flat_file_objs_all:
        ...
        for ff_obj in flat_file_objs_all: 
            if ff_obj and ff_obj.name and os.path.exists(ff_obj.name):
                _, header = load_single_fits_from_path(ff_obj.name)
                if header:
        ...
        for filter_name, info_dict in flat_files_info_grouped_by_filter.items():
            ...
            try:
                status_messages.append(f"Master FLAT ({filter_name}): {len(flat_paths_list)}개 파일로 생성 시작...")
                # create_master_flat는 (data_np, header) 반환
                master_flat_data_np, first_flat_header = create_master_flat(
                    flat_paths_list, 
                    master_bias_data_np, 
                    master_darks_data_for_flat_calib_dict # NumPy 배열과 헤더 딕셔너리 전달
                ) 
                if master_flat_data_np is not None:
                    header_to_save = first_flat_header if first_flat_header else first_header_in_group if first_header_in_group else fits.Header()
                    if 'FILTER' not in header_to_save: header_to_save['FILTER'] = filter_name

                    base_fn = f"master_flat_{filter_name}_numpy" 
                    saved_path = save_fits_image(master_flat_data_np, header_to_save, base_fn, temp_dir, current_timestamp_str)
                    if saved_path:
                        state_master_flats_dict_out[filter_name] = saved_path 
                        ui_flats_paths_out.append(saved_path)
                        status_messages.append(f"Master FLAT ({filter_name}): {os.path.basename(saved_path)} 생성 완료 (경로: {saved_path})")
                    else: status_messages.append(f"Master FLAT ({filter_name}): 생성 실패 (저장 오류).")
                else: status_messages.append(f"Master FLAT ({filter_name}): 생성 실패.")
            except Exception as e_mf_grp: logger_ui.error(f"Master FLAT ({filter_name}) 처리 중 오류", exc_info=True); status_messages.append(f"Master FLAT ({filter_name}) 오류: {str(e_mf_grp)}")
        if not ui_flats_paths_out: status_messages.append("유효한 Master Flat 생성 실패 또는 처리할 파일 없음.")
    ...
    if not state_master_darks_dict_out and dark_file_objs: all_masters_created_successfully = False # 수정: 변수명 일치
    if not state_master_flats_dict_out and flat_file_objs_all: all_masters_created_successfully = False 
    ...
    return ui_bias_path_out, ui_darks_paths_out, ui_flats_paths_out, \
           state_bias_path_out, state_master_darks_dict_out, state_master_flats_dict_out, \
           final_status
...
def handle_tab2_light_frame_calibration(
    ...
    state_mb_p, state_md_dict_corr, state_master_flats_dict, # state_prelim_mf_dict -> state_master_flats_dict
    preview_stretch_type, preview_asinh_a,
    temp_dir):
    """
    탭 2: LIGHT 프레임 보정 핸들러 (NumPy 수동 보정).
    """
    status_messages = []
    ...
    if tab2_uploaded_bias_obj and tab2_uploaded_bias_obj.name:
        ...
        if mb_data_temp is not None: final_mb_data = mb_data_temp
    if final_mb_data is None: status_messages.append("경고: 사용 가능한 Master BIAS 없음.")
    else: status_messages.append(f"Master BIAS 사용 준비 완료.")
    ...
    # 3. 탭2에서 업로드된 필터별 최종 Master Flat 로드 (NumPy 딕셔너리)
    tab2_uploaded_master_flats_data_dict = {} 
    for filt_char, uploaded_mf_obj in [('B', tab2_uploaded_flat_b_obj), ('V', tab2_uploaded_flat_v_obj)]:
        if uploaded_mf_obj and uploaded_mf_obj.name:
            ...
            if mf_data_raw is not None:
                tab2_uploaded_master_flats_data_dict[filt_char] = (mf_data_raw, mf_header)
                status_messages.append(f"탭2 업로드 Master FLAT {filt_char} 사용 준비 완료.")
    ...
    for i, light_file_obj in enumerate(light_file_objs_list):
        ...
        try:
            ...
            status_messages.append(f"--- {light_filename} 보정 중 (NumPy 수동) ---")
            ...
            # 최종 Master FLAT 결정 (NumPy 배열과 헤더)
            final_mf_data_for_light = None; final_mf_header_for_light = None
            if current_light_filter in tab2_uploaded_master_flats_data_dict: 
                final_mf_data_for_light, final_mf_header_for_light = tab2_uploaded_master_flats_data_dict[current_light_filter]
            elif state_master_flats_dict: 
                master_flat_path = state_master_flats_dict.get(current_light_filter)
                if not master_flat_path: master_flat_path = state_master_flats_dict.get('Generic')
                if master_flat_path and os.path.exists(master_flat_path):
                    mf_data, mf_hdr = load_single_fits_from_path(master_flat_path, f"탭1 Master Flat {current_light_filter or 'Generic'}")
                    if mf_data is not None: 
                        final_mf_data_for_light = mf_data
                        final_mf_header_for_light = mf_hdr
            
            # 수동 보정 함수 호출
            calibrated_light_data_np, calib_log_list = calibrate_light_numpy(
                raw_light_data_np=light_data_raw_np,
                raw_light_header=light_header,
                master_bias_data_np=final_mb_data,
                master_dark_data_dict=state_md_dict_corr, # 경로 딕셔너리 전달 (내부에서 로드 또는 직접 데이터 전달하도록 수정)
                                                          # 여기서는 md_to_use_data_light를 직접 전달하는 것이 더 명확
                # master_dark_data_np=md_to_use_data_light, # 이렇게 변경하는 것이 더 적합
                # master_dark_header=md_to_use_header_light,
                master_flat_data_dict=state_master_flats_dict, # 경로 딕셔너리 전달
                # master_flat_data_np=final_mf_data_for_light, # 이렇게 변경하는 것이 더 적합
                # master_flat_header=final_mf_header_for_light,
                do_bias_correction=True, 
                do_dark_correction=True, 
                do_flat_correction=True  
            )
            # calibrate_light_numpy 호출 시 master_dark_data_dict와 master_flat_data_dict는
            # 경로 딕셔너리가 아닌, {exp: (data, hdr)} 와 {filt: (data, hdr)} 형태의 데이터 딕셔너리를 기대함.
            # 따라서, 탭2에서 준비된 로컬 딕셔너리를 전달하거나, calibrate_light_numpy 내부에서 경로를 처리하도록 수정 필요.
            # 여기서는 calibrate_light_numpy가 경로를 처리한다고 가정하고 state_md_dict_corr와 state_master_flats_dict를 전달.
            # 하지만, 더 나은 방식은 md_to_use_data_light와 final_mf_data_for_light를 직접 전달하는 것.
            # 아래는 후자의 방식으로 수정
            
            temp_master_darks_data_dict = {}
            if md_to_use_data_light is not None and md_to_use_header_light is not None:
                exp_key = get_fits_keyword(md_to_use_header_light, ['EXPTIME', 'EXPOSURE'], data_type=float)
                if exp_key is not None and exp_key > 0:
                    temp_master_darks_data_dict[exp_key] = (md_to_use_data_light, md_to_use_header_light)

            temp_master_flats_data_dict = {}
            if final_mf_data_for_light is not None and final_mf_header_for_light is not None:
                filt_key = get_fits_keyword(final_mf_header_for_light, ['FILTER'], 'Generic').upper()
                temp_master_flats_data_dict[filt_key] = (final_mf_data_for_light, final_mf_header_for_light)
            ...
            calibrated_light_data_np, calib_log_list = calibrate_light_numpy(
                ...
                raw_light_header=light_header,
                master_bias_data_np=final_mb_data,
                master_dark_data_dict=temp_master_darks_data_dict, # LIGHT 프레임 노출시간에 맞는 DARK만 전달
                master_flat_data_dict=temp_master_flats_data_dict, # LIGHT 프레임 필터에 맞는 FLAT만 전달
                do_bias_correction=True, 
                do_dark_correction=True, 
                do_flat_correction=True  
            )
            ...
            processing_history = f'Manually Calibrated (NumPy) App v0.18 (B:{final_mb_data is not None}, D:{md_to_use_data_light is not None}, F:{final_mf_data_for_light is not None})'
            light_header['HISTORY'] = str(light_header.get('HISTORY','')) + '; ' + processing_history
...
def handle_tab3_extinction_analysis(
    ...
    state_mb_p, state_md_dict_corr, state_master_flats_dict, 
    star_detection_thresh_factor,
    temp_dir):
    """
    탭 3: 대기소광계수 분석 핸들러 (NumPy 수동 보정).
    """
    status_log = []
    ...
    if uploaded_mb_path_obj and uploaded_mb_path_obj.name:
        mb_data_temp, _ = load_single_fits_from_path(uploaded_mb_path_obj.name, "탭3 업로드 BIAS")
        if mb_data_temp is not None: final_mb_data = mb_data_temp
    elif state_mb_p and os.path.exists(state_mb_p):
        mb_data_temp, _ = load_single_fits_from_path(state_mb_p, "탭1 BIAS")
        if mb_data_temp is not None: final_mb_data = mb_data_temp
    ...
    if uploaded_md_raw_files_objs: 
        for dark_file_obj in uploaded_md_raw_files_objs:
            if dark_file_obj and dark_file_obj.name:
                raw_md_data, raw_md_header = load_single_fits_from_path(dark_file_obj.name, f"탭3 업로드 Raw DARK")
                if raw_md_data is not None and raw_md_header is not None:
                    exp_time = get_fits_keyword(raw_md_header, ['EXPTIME', 'EXPOSURE'], -1.0, float)
                    if exp_time > 0:
                        corrected_dark_data = raw_md_data.copy()
    ...
    tab3_uploaded_master_flats_data_dict = {} 
    for filt_char, uploaded_mf_obj in [('B', uploaded_mf_b_raw_path_obj), ('V', uploaded_mf_v_raw_path_obj)]:
        if uploaded_mf_obj and uploaded_mf_obj.name:
            mf_data_raw, mf_header = load_single_fits_from_path(uploaded_mf_obj.name, f"탭3 업로드 Master FLAT {filt_char}")
            if mf_data_raw is not None:
                tab3_uploaded_master_flats_data_dict[filt_char] = (mf_data_raw, mf_header)
    ...
    for light_file_obj in light_file_objs: 
        ...
        try:
            ...
            if current_filter in tab3_uploaded_master_flats_data_dict:
                final_mf_data_for_light, final_mf_header_for_light = tab3_uploaded_master_flats_data_dict[current_filter]
            elif state_master_flats_dict:
                master_flat_path = state_master_flats_dict.get(current_filter)
                if not master_flat_path: master_flat_path = state_master_flats_dict.get('Generic')
                if master_flat_path and os.path.exists(master_flat_path):
                    mf_data, mf_hdr = load_single_fits_from_path(master_flat_path, f"탭1 Master Flat {current_filter or 'Generic'}")
                    if mf_data is not None: final_mf_data_for_light, final_mf_header_for_light = mf_data, mf_hdr
            
            # 수동 보정
            calibrated_data_np, calib_log_list = calibrate_light_numpy(
                light_data_raw_np, light_header,
                master_bias_data_np=final_mb_data,
                master_dark_data_dict= {current_exptime: (md_to_use_data_light, md_to_use_header_light)} if md_to_use_data_light is not None else None,
                master_flat_data_dict= {current_filter: (final_mf_data_for_light, final_mf_header_for_light)} if final_mf_data_for_light is not None else None
            )
            status_log.extend(calib_log_list)
            
            sources_table = detect_stars_extinction(calibrated_data_np, fwhm_dao=fwhm_for_dao_tab3, threshold_nsigma_dao=float(star_detection_thresh_factor))
...
def handle_tab4_detailed_photometry(
    ...
    current_timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    logger_ui.info("탭 4 상세 측광 분석 시작 (보정된 파일 사용 가정)...")
    ...
    # --- 2. 표준별 처리로 유효 영점(m0_eff) 계산 (보정된 파일 사용) ---
    ...
    # --- 4. 별 정보 통합, B-V 계산, SIMBAD 질의, 정렬 (이전과 동일) ---
    final_display_list = []
